...
void static commonInit(const uint8_t *cmdList) {
  ...
  EUSCI_A3->BRW = 3;        //change to 1 when on 3mhz (originally 3)
...
uint32_t ST7735_DrawString(uint16_t x, uint16_t y, char *pt, int16_t textColor){
  ...
  while(*pt){
	  ...
	  ST7735_DrawCharS(x*6, y*10, *pt, textColor, ST7735_BLUE, 2);
    pt++;
    x = x+1;
...
void ST7735_OutChar(char ch){
  ...
  ST7735_DrawCharS(StX*6,StY*10,ch,StTextColor,ST7735_BLUE, 1);   //Part1
  //ST7735_DrawCharS(StX*6,StY*10,ch,StTextColor,ST7735_BLACK, 2);  //Part2
  ...
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,ST7735_BLACK, 2);
  }
  return;
}
...
void Output_Init(void){
  ...
  ST7735_FillScreen(ST7735_BLUE);                 // set screen to black
}
